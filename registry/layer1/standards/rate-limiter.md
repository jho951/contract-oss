...
- 요청 제한의 key, plan, decision, policy 계약을 제공한다.
- rate limit 계산 primitive와 외부 저장소 연동 SPI를 순수 기능으로 유지한다.
- 2계층 `platform-security`가 rate limit을 보안 runtime에 조립할 수 있게 한다.

## 계층 원칙

- `rate-limiter`는 1계층 OSS다.
- `rate-limiter`는 제한 계산과 storage SPI를 제공하고, 서비스 보안 체인 조립은 소유하지 않는다.
- 서비스 role preset, endpoint별 기본 제한, 실패 응답 표준화는 `platform-security` 책임이다.
...
- gateway / service role preset 조립
...
- 어떤 요청에 어떤 preset과 실패 응답으로 적용할지 정하면 `platform-security` 책임이다.
