...
- 타입 안전한 정책 키, 설정 source, resolver, builder primitive를 제공한다.
- 정책 설정 조회 기능을 순수 기능 모듈로 유지한다.
- 2계층 `platform-governance`가 운영 정책 조회와 fail-fast 기준을 조립할 수 있게 한다.

## 계층 원칙

- `policy-config`는 1계층 OSS다.
- `policy-config`는 정책 값을 읽고 해석하는 primitive를 제공한다.
- 정책 평가 엔진, 위반 대응, 운영 preset은 `platform-governance` 책임이다.
...
- 조회한 정책으로 운영 결정을 내리고 위반을 처리하면 `platform-governance` 책임이다.
