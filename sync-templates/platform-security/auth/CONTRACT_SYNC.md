...
- version 관련 키 이름이 다른 1계층 OSS와 일치하는가
- publish 좌표를 하드코딩하지 않는가
- `github_org`와 `github_repo`가 있는가
...
4. `gradle.properties`와 `publish.yml` 구조를 `oss-contract` 표준과 맞춘다.
5. `platform-governance`, `platform-resource` 책임을 침범하지 않는지 확인한다.
6. public 계약과 internal 구현 설명을 구분한다.
