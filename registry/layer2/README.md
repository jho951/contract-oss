...
목표 구조는 [l5-target-architecture.md](l5-target-architecture.md), 실제 전환 순서는 [l5-migration-plan.md](l5-migration-plan.md), 서비스별 체크는 [l5-service-migration-checklist.md](l5-service-migration-checklist.md)에서 관리한다.

## 대상

현재 조직 표준으로 관리하는 2계층 platform line은 아래 네 개다.
이 목록은 가능한 2계층 구현 전체를 뜻하지 않는다.
...
- 같은 1계층 OSS를 바탕으로 다른 2계층 platform line을 추가로 만들 수 있다.
- 이 문서에 나오는 `platform-security`, `platform-governance`, `platform-resource`, `platform-integrations`는 현재 조직이 표준으로 운영하는 line이다.
...
- 현재 조직 표준 security line인 `platform-security`는 `platform-security-client`를 outbound propagation add-on으로 두고, `platform-security-auth-bridge-starter`, `platform-security-ratelimit-bridge-starter`, `platform-security-hybrid-web-adapter`를 sanctioned surface로 포함한다.
- `platform-governance`는 `GovernanceAuditSink`, `GovernanceAuditRecorder`, `PolicyConfigSource`, `GovernanceDecisionEngine`, `ViolationHandler`를 public SPI로 고정한다.
- `platform-governance`는 `./gradlew check -PuseLocalLayer1=true` 경로까지 유지하고, dependency locking 완화는 local source 검증 모드에서만 허용한다.
- `platform-resource`는 `platform-resource-filestorage-bridge-starter`, `platform-resource-notification-bridge-starter`를 통해 raw `FileStorage`, `NotificationDispatcher` bean을 platform port로 흡수한다.
- `platform-integrations`는 `platform-integrations-api`, `platform-integrations-notification-adapter`, `platform-integrations-autoconfigure`, `platform-integrations-starter`를 기준 surface로 두고, 서비스-facing outbound 계약을 `PlatformOutboundSender`로 고정한다.
- 2계층 완료 판정은 문서 문구만이 아니라 `verifyPublishedSurface`, `verifyStarterContract`, `verifyConsumerConformance`, `verifyStage5Contract`까지 통과하는지로 본다.
...
- 1계층 OSS는 특정 2계층 line 하나에 종속되면 안 된다.
- 2계층 published artifact는 현재 line의 배포 단위일 뿐, 다른 2계층 line 생성을 막는 의미가 아니다.
...
- 서비스는 자신이 선택한 2계층 line 하나를 소비하면 되고, `platform-security` 같은 특정 line이 유일한 선택지라는 뜻은 아니다.
...
- 각 platform repo는 published surface, starter contract, sample consumer, Stage5 contract gate를 기본 품질 기준으로 유지한다.
...
| [l5-service-migration-checklist.md](l5-service-migration-checklist.md) | 서비스 레포별로 무엇을 남기고 무엇을 플랫폼으로 올릴지 | 플랫폼 공통 기준 재정의 |
